...
```{r}
library(survival)
Surv(pbc_312$time, pbc_312$status)

# Simple Kaplan Meier plot

mykm1 <- survfit(Surv(time, status) ~ 1, data = pbc_312)

plot(mykm1, mark.time = TRUE)
```
```{r}
library(VIM)
mice_plot_1 <- aggr(pbc_312, col=c('navyblue','yellow'),
                    numbers=TRUE, sortVars=TRUE,
                    labels=names(pbc_312), cex.axis=.7,
                    gap=3, ylab=c("missing data: pbc_312","Pattern"))
```
```{r}
na_count <-sapply(pbc_312, function(y) sum(length(which(is.na(y)))))
...
```
```{r}
by(pbc_312, pbc_312$status, summary)
```

```{r}
by(pbc_312, pbc_312$trt, summary)
```

```{r}
mosaicplot(table(pbc_312$sex, pbc_312$trt),
  color = TRUE,
...
```
```{r}
plot(pbc_312$time, pbc_312$platelet)
plot(pbc_312$time, pbc_312$trt)
```
